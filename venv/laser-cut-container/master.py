...
hp = check_input(120, 150, "Please input a height for the pencil holder (Recommended: 100-140 mm): ")
...
def Sides():
    ...
    # CHANGE THIS SO IT ADDS THE ACTUAL PATH
    sideOutput += f'<image x="{5 + 2*l + d/2 - 10}" y="{hp/3 + 10 + yOffset}" width="20" height="22.3" xlink:href="seas.svg" />'
...
# Compile all paths into one svg
def compile():
    final = starter
    final += Spine()
    final += Base()
